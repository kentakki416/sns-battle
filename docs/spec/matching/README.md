...
- [フロー図](#フロー図)
  - [マッチング成立フロー](#マッチング成立フロー)
  - [ビデオ通話接続フロー](#ビデオ通話接続フロー)
  - [テーマ進行フロー](#テーマ進行フロー)
  - [リアクション送信・一致判定フロー](#リアクション送信一致判定フロー)
  - [スタンプ送信フロー](#スタンプ送信フロー)
  - [マッチング終了フロー](#マッチング終了フロー)
- [UI 設計](#ui-設計)
  - [画面一覧](#画面一覧)
...
機能ごとにシーケンス図を分けて記載する。**`api`** は `apps/api`（REST + SSE + LiveKit Webhook 受信）、**`worker`** は `apps/matching-worker`（BullMQ ジョブ消化）、**`Q`** は BullMQ Queue（Redis backed）、**`Redis`** は ElastiCache、**`DB`** は RDS Postgres、**`LK`** は LiveKit Cloud（Room + Data Channel）を指す。

すべての delayed job は idempotent に設計され、Job ID は `session:{sessionId}:advance:{round}` のような決定的 ID で重複 enqueue を防ぐ（[BullMQ ジョブ設計](#bullmq-ジョブ設計) 参照）。
...
sequenceDiagram
    ...
    API->>Redis: ZADD matching_queue (B, ts)
    API->>Redis: ZRANGE 0 99（待機時間順 上位 100）
    API->>DB: blocks / matching_preferences 一括取得（N+1 回避）
    API->>API: 双方向 preference 適合チェック
    ...
    Note over A,B: 成立画面 → カウントダウン → ビデオ通話接続フローへ
```
...
カウントダウン後、両ユーザーが LiveKit Room へ接続し、`matching-worker` にテーマ進行ジョブが enqueue されるまで。
...
sequenceDiagram
    ...
    participant Q as BullMQ (theme-progress)

    A->>API: POST /api/matching/token { sessionId }
    API->>DB: matching_sessions 取得 + 参加者検証
    API->>API: VideoGrant 付き JWT 生成（canPublish/Subscribe/PublishData）
    API-->>A: { token, url }
    B->>API: POST /api/matching/token { sessionId }
    API-->>B: { token, url }

    A->>LK: Room 接続 matching:{sessionId}
    B->>LK: Room 接続 matching:{sessionId}
    LK->>API: webhook participant_joined（両者）
    API->>API: 両者 join 検知
    API->>DB: matching_sessions.status = ACTIVE, started_at = now
    API->>Q: enqueue advance-theme (round=1, delay=0s)
    API->>Q: enqueue publish-timer (delay=30s)
    API->>Q: enqueue session-timeout (delay=600s)
```

`session-timeout` / `advance-theme` / `publish-timer` の 3 系統がここで一斉に登録される。以降は `matching-worker` がジョブを消化していく。

### テーマ進行フロー

`matching-worker` が `theme-progress` キューの delayed job を消化し、テーマと残り時間を Data Channel で配信する。`advance-theme` は次ラウンドの delayed job を再 enqueue する自己ループ構造。
...
sequenceDiagram
    ...
    loop 10 ラウンド（round=1..10）
        Q-->>W: advance-theme { sessionId, round=N }
        W->>DB: matching_sessions 確認
        alt 既に ENDED or round が進んでいる
            Note over W: no-op（冪等）
        else ACTIVE かつ round=N が未配信
            W->>DB: 次テーマ取得 (talk_themes)
            W->>DB: matching_sessions 更新 (currentRound, themeId, speaker)
            W->>LK: publishData matching:theme (type, title, choices?, speaker, duration, round)
            W->>LK: publishData matching:hype (盛り上げコメント)
            LK-->>A: matching:theme + matching:hype
            LK-->>B: matching:theme + matching:hype
            W->>Q: enqueue advance-theme { round=N+1, delay=duration秒 }
        end
    end

    par 30 秒ごとの残り時間配信（並行）
        Q-->>W: publish-timer { sessionId, tickIndex }
        W->>LK: publishData matching:timer (lossy, remainSec, canEnd)
        W->>Q: enqueue publish-timer { tickIndex+1, delay=30s }
    end
```

ジョブ ID は `session:{sessionId}:advance:{round}` / `session:{sessionId}:timer:{tickIndex}` で固定化され、worker が落ちて再キューされても重複処理されない。

### リアクション送信・一致判定フロー

CHOICE テーマで両ユーザーが選択肢を選んだ瞬間に、サーバーが一致判定して結果を Data Channel で配信する。後出しユーザーへのレスポンスで初めて `isMatch` が確定する。
...
sequenceDiagram
    ...
    A->>API: POST /api/matching/sessions/:id/reaction { themeId, choiceId, round }
    API->>DB: insert matching_reactions（@@unique で 1 round 1 回答保証）
    API->>DB: 相手の同 round 回答を確認
    alt 相手が未回答
        API-->>A: { peerAnswered: false }
    else 相手が回答済み（後出し）
        API->>API: choiceId が一致するか判定
        API->>LK: publishData matching:reaction_match (round, isMatch, A.choice, B.choice)
        LK-->>A: matching:reaction_match
        LK-->>B: matching:reaction_match
        API-->>A: { peerAnswered: true, isMatch }
    end

    Note over A,B: 結果画面では GET /api/matching/sessions/:id/reactions で全 10 ラウンドを参照
```

UI 上は両者揃うまで相手の回答を `?` でマスクし、`matching:reaction_match` 受信と同時に同時公開する。

### スタンプ送信フロー

通話中の感情表現として、相手画面の `<StampFloatLayer>` 上にスタンプをフロート表示する。サーバー側でレート制限と所持判定を行ってから配信する。
...
sequenceDiagram
    ...
    A->>API: POST /api/matching/sessions/:id/stamp { itemId }
    API->>Redis: INCR stamp_rate:{userId}:{秒キー} (TTL 1s)
    alt 5 を超過
        API-->>A: 429 Too Many Requests
    else レート OK
        API->>DB: items 確認 (type=STAMP かつ scope に MATCHING を含む)
        opt is_premium=true
            API->>DB: user_inventory 所持確認
        end
        API->>LK: publishData matching:stamp (senderId, itemId, emoji, animationType)
        LK-->>B: matching:stamp → StampFloatLayer 表示
        LK-->>A: matching:stamp（送信側もエコーバック）
        API-->>A: 200 OK
    end
```

Spec1 では DB に永続化しない揮発リアクション。将来 `matching_stamps` テーブルを追加して分析用途に保存する余地を残す。

### マッチング終了フロー

終了トリガーは 3 つあり、いずれも最終的に `matching_sessions.status = ENDED` 更新と `matching:ended` Data Channel 配信、`deleteRoom` に収束する。

#### パターン1: 10 分タイムアウト

`matching-worker` が `session-timeout` ジョブを消化して終了させる。
...
sequenceDiagram
    ...
    Note over Q: ビデオ通話接続フローで 600s 後に enqueue 済み
    Q-->>W: session-timeout { sessionId }
    W->>DB: matching_sessions 取得
    alt 既に ENDED
        Note over W: no-op（冪等）
    else ACTIVE のまま
        W->>DB: status=ENDED, end_reason=TIMEOUT, ended_at=now
        W->>LK: publishData matching:ended { reason: TIMEOUT }
        W->>LK: deleteRoom matching:{sessionId}
    end
```

#### パターン2: 手動終了（5 分以降）

ユーザーが「終了」ボタンを押した場合。`api` が同期的に終了処理を行い、関連ジョブを BullMQ から除去する。
...
sequenceDiagram
    ...
    A->>API: POST /api/matching/sessions/:id/end
    API->>DB: matching_sessions 取得
    alt started_at から 5 分未満
        API-->>A: 400 Bad Request
    else 5 分以上経過
        API->>DB: status=ENDED, end_reason=MANUAL, ended_at=now
        API->>Q: remove session-timeout / advance-theme / publish-timer ジョブ
        API->>LK: publishData matching:ended { reason: MANUAL }
        API->>LK: deleteRoom matching:{sessionId}
        API-->>A: 200 OK
    end
```

#### パターン3: 相手の離脱（LiveKit Webhook 経由）

ブラウザクローズや回線切断は LiveKit Cloud から Webhook で通知される。`api` は signature 検証 + BullMQ enqueue のみを行い、`matching-worker` が冪等に終了処理を走らせる。
...
sequenceDiagram
    ...
    participant Q as BullMQ (webhook-events)
    participant W as matching-worker
    participant DB as Postgres

    B--xLK: ブラウザクローズ / 回線切断
    LK->>API: POST /api/matching/livekit-webhook (participant_left)
    API->>API: signature 検証
    API->>Q: enqueue livekit-event (jobId=event.id)
    API-->>LK: 200 OK
    Q-->>W: livekit-event
    W->>DB: matching_sessions 取得
    alt 既に ENDED
        ...
        W->>DB: status=ENDED, end_reason=USER_LEFT, ended_at=now
        W->>LK: publishData matching:ended { reason: USER_LEFT }
        W->>LK: deleteRoom matching:{sessionId}
    end
```

両者切断 / `room_finished` イベントが先に届いた場合も同じ経路で `USER_LEFT` として確定する。`jobId=event.id` により Webhook 再送による重複も冪等に吸収される。

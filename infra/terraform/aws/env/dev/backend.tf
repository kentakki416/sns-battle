...
terraform {
  backend "s3" {
    bucket         = "sns-battle-terraform-state-20250101" # TODO: backet名を適切な値に変更してください
    key            = "dev/terraform.tfstate"
    region         = "ap-northeast-1"
    dynamodb_table = "sns-battle-terraform-state-lock" # TODO: bootstrap/variables.tfと同じ値に変更してください
    encrypt        = true
  }
}
